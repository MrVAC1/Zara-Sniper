...
export const getTimeConfig = () => ({
  SNIPER_INTERVAL: parseInt(process.env.SNIPER_INTERVAL) || 60000,
  GOTO_TIMEOUT: parseInt(process.env.GOTO_TIMEOUT) || 60000,
  SELECTOR_TIMEOUT: parseInt(process.env.SELECTOR_TIMEOUT) || 15000,
  ACTION_PAUSE: parseInt(process.env.ACTION_PAUSE) || 4500,
  CLICK_DELAY: parseInt(process.env.CLICK_DELAY) || 400,
  ...
  TIMEOUT_SIZE_MENU: parseInt(process.env.TIMEOUT_SIZE_MENU) || 1500,
  DELAY_POST_RELOAD: parseInt(process.env.DELAY_POST_RELOAD) || 2000,
  DELAY_BETWEEN_CONTINUE: parseInt(process.env.DELAY_BETWEEN_CONTINUE) || 2000,
  TIMEOUT_3DS_REDIRECT: parseInt(process.env.TIMEOUT_3DS_REDIRECT) || 15000,
  DELAY_POST_CVV: parseInt(process.env.DELAY_POST_CVV) || 5000,
  DELAY_CAPTCHA_SOLVE: parseInt(process.env.DELAY_CAPTCHA_SOLVE) || 30000,
  TIMEOUT_API_RETRY: parseInt(process.env.TIMEOUT_API_RETRY) || 5000,
  TIMEOUT_HEALTH_PAGE: parseInt(process.env.TIMEOUT_HEALTH_PAGE) || 60000,
  DELAY_3DS_SUCCESS: parseInt(process.env.DELAY_3DS_SUCCESS) || 3000,

  // Supplemental Timing Variables
  TIMEOUT_CLICK_TRIAL: parseInt(process.env.TIMEOUT_CLICK_TRIAL) || 500,
  TIMEOUT_DB_RETRY: parseInt(process.env.TIMEOUT_DB_RETRY) || 5000,
  TIMEOUT_LOOP_RETRY: parseInt(process.env.TIMEOUT_LOOP_RETRY) || 5000,
  DELAY_WATCH_LOOP: parseInt(process.env.DELAY_WATCH_LOOP) || 1000,
  TIMEOUT_FAST_SELECTOR: parseInt(process.env.TIMEOUT_FAST_SELECTOR) || 3000,
  TIMEOUT_SOLD_OUT_CHECK: parseInt(process.env.TIMEOUT_SOLD_OUT_CHECK) || 10000,
  TIMEOUT_MODAL_CHECK: parseInt(process.env.TIMEOUT_MODAL_CHECK) || 600,
  ...
  DELAY_CHECKOUT_STEP: parseInt(process.env.DELAY_CHECKOUT_STEP) || 300,
  DELAY_FAST_BACKTRACK: parseInt(process.env.DELAY_FAST_BACKTRACK) || 200,
  TIMEOUT_PAY_BUTTON: parseInt(process.env.TIMEOUT_PAY_BUTTON) || 2000,

  // Hybrid Sniper Config
  API_MONITORING_INTERVAL: parseInt(process.env.API_MONITORING_INTERVAL) || 2000,
